...
class Training_Model:
    ...
    def Saving_MoldelPickle(self):
        X_SoilType = np.array(self.DP.df[self.DP.columns[:-2]])
        Y_SoilType = np.array(self.DP.df['Soil_Type'])
        X_Cover = np.array(self.DP.df[self.DP.columns[:-1]])
...
class Use_Model:
    ...
    def Predict_Soil(self, inputFeatures):
        ...
        Pre_Y = self.Soil_Model.predict(inputFeatures)[0]
        return Pre_Y

    def Predict_Cover(self, inputFeatures):
        inputFeatures = np.array([inputFeatures])
        Pre_Y = self.Cover_Model.predict(inputFeatures)[0]
        return Pre_Y
